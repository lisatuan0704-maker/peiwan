...
(() => {
 ...
 window.ttLiveOpen=({view,cid,mode}={})=>{if(view==='roster')rosterMode=mode==='book'?'book':'now';document.querySelectorAll('.live-choose-backdrop').forEach(x=>x.remove());refresh(true);if(view==='profile')openInfo(cid);else if(view==='shop'||view==='bag'){document.querySelector('#shopWindow').dataset.area=view;const title=document.querySelector('#shopTitle');title.textContent=view==='bag'?'我的背包':'時裝間';title.dataset.text=title.textContent;document.querySelector('#shopWindow .close').setAttribute('aria-label','關閉'+title.textContent);document.querySelector('#shopFrame').title=view==='bag'?'個人背包與收藏':'時裝商品與試穿';document.querySelector('#shopWindow .window-head small').textContent=view==='bag'?'MY LITTLE ARCHIVE / 把喜歡的收藏好':'TAVERN BOUTIQUE / 選一份新的心情';show('shop',false);const f=document.querySelector('#shopFrame');f.dataset.liveArea=view;f.contentWindow.ttDressArea?.(view==='bag'?'bag':'store');}else show('roster',false);};
